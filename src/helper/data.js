const text = [
  `
Odaklanmak, 1000 iyi fikre hayır diyebilmektir.-Steve Jobs`,
  `İnsanlar, yazılımın ürün olmadığını, ürün üretmek için kullanıldığını anlayacaklar.
Linus Torvalds.`,
  `Yazılım kalitesini test miktarını artırarak iyileştirmeye çalışmak, kendinizi daha sık tartarak kilo vermeye çalışmak gibidir. Teraziye adım atmadan önce ne yediğiniz, ne kadar tartacağınızı belirler ve kullandığınız yazılım geliştirme teknikleri, testte kaç hata bulacağını belirler. - Steve McConnell `,
  `Programlama, ne bildiğinizle değil; neyi çözebildiğinizle ilgilidir.`,
  `İlk önce, sorunu çözün. Ardından kodu yazın.`,
  `Dil seçerken, aslında seçtiğiniz teknik fayda setinden fazlasıdır. Bir topluluk seçersiniz.`,
  `En iyi öğrenenler, bir konuda nesnel şekilde kötü olmalarının verdiği rahatsızlığa rağmen peşini bırakmayarak sonuna kadar gidenlerdir.`,
 
];
export default text;
